...
import { getOrderDetails } from '@/services/order/Order';
...
const OrderDetails = () => {
  ...
  const params = useParams(); //or you destructure and get params value

  const router = useRouter();

  const extractOrderDetails = useCallback(async () => {
    ...
    console.log(res);
  }, [params, setOrderDetails, setPageLevelLoader]);
  ...
  return (
    <div className='py-14 px-4 md:px-6'>
      <div className='flex justify-start items-start space-y-2 flex-col'>
        <h1 className='text-xl lg:text-4xl font-bold leading-7 lg:leading-9 text-gray-900'>
          ...
          <div className='flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:p-6 xl:p-8 w-full'>
            ...
            orderDetails.orderItems.length
              ? orderDetails.orderItems.map((item) => (
                  ...
                  >
                    <div className='pb-4 md:pb-8 w-full h-3/4 md:max-h-[30rem] md:w-40'>
                      <Image
                        height={500}
                        width={500}
                        priority={true}
